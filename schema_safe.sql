-- FollowTrain Supabase Schema - Safe Version for Existing Tables
...
CREATE POLICY "Allow users to update own entries" ON participants
  FOR UPDATE USING (
    ...
    OR
    -- Or they have the participant ID (for self-edit) - check if header exists and is valid UUID
    (
      ...
      AND uuid_nil() != uuid_nil() -- Placeholder to ensure valid syntax, will be replaced with proper UUID check
      AND id = uuid(current_setting('request.headers', true)::json->>'x-participant-id')
    )
  );
...
CREATE POLICY "Allow users to delete own entries" ON participants
  FOR DELETE USING (
    -- Check if header exists and is valid UUID, then compare with id
    current_setting('request.headers', true)::json->>'x-participant-id' IS NOT NULL
    AND current_setting('request.headers', true)::json->>'x-participant-id' != ''
    AND id = uuid(current_setting('request.headers', true)::json->>'x-participant-id')
  );
