-- FollowTrain Supabase Schema - Production Ready Version
...
DROP POLICY IF EXISTS "Allow all for trains" ON trains CASCADE;
DROP POLICY IF EXISTS "Allow all for participants" ON participants CASCADE;
...
CREATE POLICY "Allow users to update own entries" ON participants
  FOR UPDATE USING (
    ...
    -- Use a condition that safely compares the ID when the header exists
    (
      ...
      AND EXISTS (
        SELECT 1 WHERE 
          ...
          AND id = CAST(current_setting('request.headers', true)::json->>'x-participant-id' AS UUID)
      )
    )
  );
