...
-- Create activity_log table for tracking train activity
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'activity_log') THEN
        CREATE TABLE public.activity_log (
          ...
          action_type VARCHAR(50) NOT NULL, -- 'join', 'leave', 'update', 'lock', 'unlock'
          action_details JSONB, -- Additional details about the action
          created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
        );
        RAISE NOTICE 'Table activity_log created';
    ELSE
        RAISE NOTICE 'Table activity_log already exists';
    END IF;
END $$;

-- Create analytics table for tracking profile views and clicks
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'analytics') THEN
        CREATE TABLE public.analytics (
          ...
          train_id VARCHAR(6) REFERENCES public.trains(id),
          participant_id UUID REFERENCES public.participants(id),
          metric_type VARCHAR(50) NOT NULL, -- 'profile_view', 'social_click', 'export'
          platform VARCHAR(20), -- For social_click events
          created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
        );
        RAISE NOTICE 'Table analytics created';
    ELSE
        RAISE NOTICE 'Table analytics already exists';
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Allow read access to trains" ON public.trains;
DROP POLICY IF EXISTS "Allow train creation" ON public.trains;
DROP POLICY IF EXISTS "Allow train creation with validation" ON public.trains;
DROP POLICY IF EXISTS "Allow train hosts to update" ON public.trains;
DROP POLICY IF EXISTS "Allow train hosts to delete" ON public.trains;
DROP POLICY IF EXISTS "Allow all for trains" ON public.trains;

DROP POLICY IF EXISTS "Allow reading train participants" ON public.participants;
DROP POLICY IF EXISTS "Allow joining trains" ON public.participants;
DROP POLICY IF EXISTS "Allow joining trains with validation" ON public.participants;
DROP POLICY IF EXISTS "Allow users to update own entries" ON public.participants;
DROP POLICY IF EXISTS "Allow hosts to remove participants" ON public.participants;
DROP POLICY IF EXISTS "Allow users to delete own entries" ON public.participants;
DROP POLICY IF EXISTS "Allow all for participants" ON public.participants;

DROP POLICY IF EXISTS "Allow reading activity logs" ON public.activity_log;
DROP POLICY IF EXISTS "Allow creating activity logs" ON public.activity_log;

DROP POLICY IF EXISTS "Allow reading analytics" ON public.analytics;
DROP POLICY IF EXISTS "Allow creating analytics" ON public.analytics;
...
-- Allow reading activity logs for train participants
CREATE POLICY "Allow reading activity logs" ON public.activity_log
  FOR SELECT USING (
    train_id IN (
      SELECT train_id FROM public.participants 
      WHERE id = (
        SELECT id FROM public.participants 
        WHERE train_id = public.activity_log.train_id 
        LIMIT 1
      )
    )
  );

-- Allow creating activity logs (application controlled)
CREATE POLICY "Allow creating activity logs" ON public.activity_log
  FOR INSERT WITH CHECK (true);

-- Analytics policies
-- Allow reading analytics for train participants
CREATE POLICY "Allow reading analytics" ON public.analytics
  FOR SELECT USING (
    train_id IN (
      SELECT train_id FROM public.participants 
      WHERE id = (
        SELECT id FROM public.participants 
        WHERE train_id = public.analytics.train_id 
        LIMIT 1
      )
    )
  );

-- Allow creating analytics (application controlled)
CREATE POLICY "Allow creating analytics" ON public.analytics
  FOR INSERT WITH CHECK (true);
...
-- Enable Realtime on activity_log table
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'activity_log') THEN
        ...
        RAISE NOTICE 'Added activity_log table to supabase_realtime publication';
    ELSE
        RAISE NOTICE 'activity_log table already exists in supabase_realtime publication';
    END IF;
END $$;
...
RETURNS void AS $$
BEGIN
  -- Delete analytics of expired trains
  DELETE FROM public.analytics WHERE train_id IN (
    SELECT id FROM public.trains WHERE expires_at < NOW()
  );
  
  -- Delete activity logs of expired trains
  DELETE FROM public.activity_log WHERE train_id IN (
    SELECT id FROM public.trains WHERE expires_at < NOW()
  );
...
RETURNS void AS $$
BEGIN
  INSERT INTO public.activity_log (train_id, participant_id, action_type, action_details)
  VALUES (p_train_id, p_participant_id, p_action_type, p_action_details);
END;
$$ LANGUAGE plpgsql 
SECURITY DEFINER
SET search_path = public, pg_catalog;

-- Create function to log analytics
...
RETURNS void AS $$
BEGIN
  INSERT INTO public.analytics (train_id, participant_id, metric_type, platform)
  VALUES (p_train_id, p_participant_id, p_metric_type, p_platform);
END;
$$ LANGUAGE plpgsql 
SECURITY DEFINER
SET search_path = public, pg_catalog;
